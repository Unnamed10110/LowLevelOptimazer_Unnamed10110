...
where gcc >nul 2>&1
if %ERRORLEVEL% EQU 0 (
    gcc -Wall -Wextra -O2 -o win_optimizer.exe win_optimizer.c -lshell32 -ladvapi32 -lgdi32
    if %ERRORLEVEL% EQU 0 (
    ...
    where cl >nul 2>&1
    if %ERRORLEVEL% EQU 0 (
        cl /nologo /W3 /O2 /DUNICODE /D_UNICODE win_optimizer.c shell32.lib advapi32.lib gdi32.lib /Fe:win_optimizer.exe
        if %ERRORLEVEL% EQU 0 (
            echo Build successful: win_optimizer.exe
